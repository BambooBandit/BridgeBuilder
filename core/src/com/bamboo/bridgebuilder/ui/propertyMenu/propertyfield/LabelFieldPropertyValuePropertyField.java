                ...
                else if(map.spriteMenu.selectedSpriteTools.size == 0)
                {
                    for (int i = 0; i < map.selectedSprites.size; i++)
                    {
                        ...
                        if (Utils.containsEquivalentPropertyField(map.selectedSprites.get(i).instanceSpecificProperties, thisProperty))
                            propertyField = (LabelFieldPropertyValuePropertyField) map.selectedSprites.get(i).instanceSpecificProperties.get(Utils.indexOfEquivalentProperty(map.selectedSprites.get(i).instanceSpecificProperties, thisProperty));
                        if (propertyField != null)
                        {
                            final LabelFieldPropertyValuePropertyField finalPropertyField = propertyField;
                            textFieldActions.add(() ->
